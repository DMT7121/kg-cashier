// Cloudflare Worker — Standalone CUKCUK API CORS Proxy & Token Manager
...
// Fetch CUKCUK credentials (prioritize env/secrets, fallback to secure GAS check)
...
export default {
  async fetch(request, env, ctx) {
    ...
    const pathSegments = url.pathname.startsWith('/') ? url.pathname.slice(1).split('/') : url.pathname.split('/');
